...
use crate::transcript::{Challenge, ChallengeScalarType, ChallengeSpace, TranscriptRead};
...
pub fn verify_proof<'a, C: CurveAffine, S: ChallengeSpace<C>, T: TranscriptRead<C, S>>(
    ...
    let iota = *transcript.squeeze_challenge_scalar(ChallengeScalarType::Default);

    msm.append_term(iota, s_poly_commitment);

    let z = *transcript.squeeze_challenge_scalar(ChallengeScalarType::Default);
    ...
    for _ in 0..k {
        ...
        let challenge =
            *transcript.to_challenge_scalar(challenge_packed, ChallengeScalarType::Default);
