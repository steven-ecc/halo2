...
impl<F: FieldExt, L: Layouter<F>> MessageScheduleConfig<'_, F, L> {
        ...
        Ok(input
            ...
            .map(|(idx, word)| {
                ...
                let subregion1_word = self
                    .decompose_subregion1_word(region_index, word.value.unwrap(), idx + 1)
                    .unwrap();

                // lower_sigma_0 on W_[1..14]
                self.lower_sigma_0(region_index, subregion1_word).unwrap()
            })
            .collect::<Vec<_>>())
    }
    ...
    fn decompose_subregion1_word(
        ...
        self.layouter().assign_existing_region(
            ...
            |mut region: Region<'_, Self>| {
                ...
                let spread_c = SpreadVar::with_lookup(
                    &mut region,
    ...
    fn lower_sigma_0(
        ...
        let (spread_a, spread_b_lo, spread_b_hi) = self.layouter().assign_existing_region(
            region_index,
            || "lower_sigma_0",
            |mut region: Region<'_, Self>| {
                // Witness `spread_a`
