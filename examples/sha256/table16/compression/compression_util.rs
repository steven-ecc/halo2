...
// Rows needed for init subregion
pub const SUBREGION_INIT_ROWS: usize = DECOMPOSE_ABCD * 4 + DECOMPOSE_EFGH * 4;
...
pub fn get_round_row(round_idx: i32) -> usize {
    ...
    if round_idx == -1 {
        ...
        // Main subregion
        SUBREGION_INIT_ROWS + (round_idx as usize) * SUBREGION_MAIN_WORD
    }
}
...
pub fn get_digest_abcd_row() -> usize {
    SUBREGION_INIT_ROWS + SUBREGION_MAIN_ROWS
}
...
impl<F: FieldExt, L: Layouter<F>> CompressionConfig<'_, F, L> {
    ...
    > {
        ...
        self.layouter().assign_existing_region(
            ...
            |mut region: Region<'_, Self>| {
                ...
                let c_lo =
                    SpreadVar::without_lookup(&mut region, a_3, row, a_4, row, a_spread_pieces[2])?;
                let c_mid =
                    SpreadVar::without_lookup(&mut region, a_5, row, a_6, row, a_spread_pieces[3])?;
    ...
    > {
        ...
        self.layouter().assign_existing_region(
            ...
            |mut region: Region<'_, Self>| {
                ...
                let b_lo =
                    SpreadVar::without_lookup(&mut region, a_3, row, a_4, row, spread_pieces[2])?;
                let b_hi =
                    SpreadVar::without_lookup(&mut region, a_5, row, a_6, row, spread_pieces[3])?;
                ...
                let d =
                    SpreadVar::with_lookup(&mut region, &configured.lookup, row, spread_pieces[5])?;
    ...
    pub(super) fn decompose_a(
        ...
        let (dense_halves, spread_halves) = self.assign_word_halves(region_index, row, a_val)?;
        let (a, b, c_lo, c_mid, c_hi, d) = self.decompose_abcd(region_index, row, a_val)?;
    ...
    pub(super) fn decompose_e(
        ...
        let (dense_halves, spread_halves) = self.assign_word_halves(region_index, row, e_val)?;
        let (a_lo, a_hi, b_lo, b_hi, c, d) = self.decompose_efgh(region_index, row, e_val)?;
    ...
    pub(super) fn assign_upper_sigma_0(
        ...
        self.layouter().assign_existing_region(
            region_index,
            || "assign upper sigma 0",
            |mut region: Region<'_, Self>| {
                region.assign_fixed(
                    || "s_upper_sigma_0",
                    configured.s_upper_sigma_0,
                    row,
                    || Ok(F::one()),
                )
            },
    ...
    pub(super) fn assign_upper_sigma_1(
        ...
        self.layouter().assign_existing_region(
            region_index,
            || "assign upper sigma 1",
            |mut region: Region<'_, Self>| {
                region.assign_fixed(
                    || "s_upper_sigma_1",
                    configured.s_upper_sigma_1,
                    row,
                    || Ok(F::one()),
                )
            },
    ...
    pub(super) fn assign_ch(
        ...
        self.layouter().assign_existing_region(
            ...
            || "assign ch",
            |mut region: Region<'_, Self>| {
                region.assign_fixed(|| "s_ch", configured.s_ch, row, || Ok(F::one()))
            },
        )?;
        ...
        self.assign_ch_outputs(region_index, row, p0_even, p0_odd, p1_even, p1_odd)
    }
    ...
    pub(super) fn assign_ch_neg(
        ...
        self.layouter().assign_existing_region(
            ...
            || "assign ch neg",
            |mut region: Region<'_, Self>| {
                region.assign_fixed(|| "s_ch_neg", configured.s_ch_neg, row, || Ok(F::one()))?;
    ...
    pub(super) fn assign_maj(
        ...
        self.layouter().assign_existing_region(
            ...
            || "assign maj",
            |mut region: Region<'_, Self>| {
                region.assign_fixed(|| "s_maj", configured.s_maj, row, || Ok(F::one()))
            },
        )?;
        ...
        self.assign_maj_outputs(region_index, row, m0_even, m0_odd, m1_even, m1_odd)
    }
    ...
    pub(super) fn assign_h_prime(
        ...
        self.layouter().assign_existing_region(
            ...
            || "assign h_prime",
            |mut region: Region<'_, Self>| {
                region.assign_fixed(|| "s_h_prime", configured.s_h_prime, row, || Ok(F::one()))?;
                ...
                region.assign_advice(
                    ...
                    a_6,
                    row,
                    || Ok(F::from_u64(k_pieces[1] as u64)),
                )?;
                ...
                Ok((
                    CellValue16::new(h_prime_lo, Some(h_prime_halves[0] as u16)),
                    CellValue16::new(h_prime_hi, Some(h_prime_halves[1] as u16)),
                ))
            },
        )
    }
    ...
    pub(super) fn assign_e_new(
        ...
        let e_new_dense =
            self.assign_word_halves_dense(region_index, row, a_8, row + 1, a_8, e_new)?;

        self.layouter().assign_existing_region(
            region_index,
            || "assign e new",
            |mut region: Region<'_, Self>| {
                region.assign_fixed(|| "s_e_new", configured.s_e_new, row, || Ok(F::one()))?;
                region.assign_advice(
                    || "e_new_carry",
                    a_9,
                    row + 1,
    ...
    pub(super) fn assign_a_new(
        ...
        let a_new_dense =
            self.assign_word_halves_dense(region_index, row, a_8, row + 1, a_8, a_new)?;

        self.layouter().assign_existing_region(
            region_index,
            || "assign a_new",
            |mut region: Region<'_, Self>| {
                region.assign_fixed(|| "s_a_new", configured.s_a_new, row, || Ok(F::one()))?;
    ...
    pub fn assign_word_halves_dense(
        ...
        self.layouter().assign_existing_region(
            ...
            |mut region: Region<'_, Self>| {
                ...
                let w_lo_cell = CellValue16::new(lo, Some(halves[0] as u16));
                let w_hi_cell = CellValue16::new(hi, Some(halves[1] as u16));
    ...
    pub fn assign_word_halves(
        ...
        self.layouter().assign_existing_region(
            ...
            || "assign word halves",
            |mut region: Region<'_, Self>| {
                let w_lo = SpreadVar::without_lookup(&mut region, a_7, row, a_8, row, w_lo)?;
                let w_hi =
                    SpreadVar::without_lookup(&mut region, a_7, row + 1, a_8, row + 1, w_hi)?;

                let w_lo_cell = CellValue16::new(w_lo.dense.cell, w_lo.dense.value);
                let w_hi_cell = CellValue16::new(w_hi.dense.cell, w_hi.dense.value);
                let spread_w_lo_cell = CellValue32::new(w_lo.spread.cell, w_lo.spread.value);
                let spread_w_hi_cell = CellValue32::new(w_hi.spread.cell, w_hi.spread.value);
