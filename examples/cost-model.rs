...
impl Circuit {
    fn proof_size(&self) -> usize {
        ...
        // Vanishing argument:
        // - (max_deg - 1) * 32 bytes (commitments) + (max_deg - 1) * 32 bytes (h_evals) for quotient polynomial
        ...
        size(1, 0) * self.advice_columns
            ...
            + self
                ...
                .sum::<usize>()
            + size(self.max_deg - 1, self.max_deg - 1)
            + size(0, self.column_queries)
            + size(1, self.point_sets)
            + size(1 + 2 * self.k, 2)
    }
}
